class JellyFish {
    ...
    addTenticles() {
       ...
       for (let i = 0; i < n; i++)
        {this.t1.push(new Particle(this.particles[13].x, this.particles[13].y + i*sp));}
      return this.t1;
    }

    connectTenticlePoints() {
        for (let i = 0; i < this.t1.length - 1; i++) {
            let particle_i = this.t1[i];
            for (let j = i + 1; j < this.t1.length; j++) {
                let particle_j = this.t1[j];
                // A Spring needs two particles, a resting length, and a strength
    ...
    // connect() {
    //     this.springs.push(new Spring(this.particles[13], this.t1[0], 0.5));
